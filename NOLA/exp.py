...
if __name__ == "__main__":
    ...
    parser.add_argument("--train_data_path", type=str, default='./data/cifar10')
    parser.add_argument("--val_data_path", type=str, default='./data/cifar10')
    ...
    parser.add_argument('--weights', type=str, default=None,
                        help='load pretrained lora/nola weights')
    parser.add_argument('--exp', type=str, default="architecture",
    ...
    if cfg.exp == "architecture":
        for type in ["full", "lora", "nola_mlp", "linear"]:
            cfg.train_type = type
            logging.info(cfg)
            acc_file = f"{cfg.outdir}/acc_file_{cfg.train_type}_kshot_{cfg.kshot}.csv"
            start = time.time()
            ...
            logging.info(f"Total trainable parameters: {num_params / 2 ** 20:.3f}M")
            ...
            logging.info(f"Classifier trainable parameters: {num_cls_params / 2 ** 20:.3f}M")
        ...
        for type in ["lora", "nola_mlp"]:
            cfg.kshot = 5
            for r in [1,2,4,8,16,32]:
                cfg.train_type = type
                cfg.rank = r
                logging.info(cfg)
                acc_file = f"{cfg.outdir}/{cfg.exp}_acc_file_{cfg.train_type}_kshot_{cfg.kshot}_rank_{cfg.rank}.csv"
                start = time.time()
                ...
                print("Rank: ", cfg.rank)
                print("Train type: ", cfg.train_type)
                num_params = sum(p.numel() for p in peft_model.parameters() if p.requires_grad)
        ...
        cfg.train_type = "nola_mlp"
        cfg.kshot = 10
        for r in [1,2,4,8,16]:
            cfg.rank = r
            for num_k in [128, 256, 512, 1024]:
                cfg.ka = num_k
                cfg.kb = num_k
                logging.info(cfg)
                acc_file = f"{cfg.outdir}/{cfg.exp}_acc_file_{cfg.train_type}_kshot_{cfg.kshot}_rank_{cfg.rank}_num_k_{cfg.ka}.csv"
                start = time.time()
        ...
        cfg.epochs = 200
        logging.info(cfg)
        acc_file = f"{cfg.outdir}/{cfg.exp}_acc_file_{cfg.train_type}_kshot_{cfg.kshot}.csv"
        start = time.time()
        ...
        raise ValueError(f"Invalid experiment: {cfg.exp}")
